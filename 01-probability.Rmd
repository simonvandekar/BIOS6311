...
These PDFs are functions with parameters:
\[
\begin{align*}
...
##### Human Connectome Project Brain volume example
...
```{r hcpVolHist, fig.width=5, fig.height=5}
hcp = read.csv('../datasets/hcp/hcp.csv')
mu = mean(hcp$FS_TotCort_GM_Vol, na.rm=T)
sigma = sd(hcp$FS_TotCort_GM_Vol, na.rm=T)
mu
sigma

x = seq(mu - 3*sigma, mu + 3*sigma, length.out=1000)
fOfX = dnorm(x, mean = mu, sd=sigma)
plot(x, fOfX, type='l', xlab='Brain volume', ylab='f(Brain Volume)')
hist(hcp$FS_TotCort_GM_Vol, xlab='Brain Volume', xlim=c(mu - 3*sigma, mu + 3*sigma))
```
...
##### Human Connectome Project CDF example (continued)
...
```{r}
# center
pnorm(550000, mean=mu, sd=sigma) - pnorm(450000, mean=mu, sd=sigma)
# lower tail
pnorm(400000, mean=mu, sd=sigma)
# upper tail
1-pnorm(600000, mean=mu, sd=sigma)
```
...
* 5% of people have a brain volume larger than what value?
* What is the middle value (median; .5 quantile) of brain volumes?
* What is interquartile range (difference between .75 quantile and .25 quantile)?

```{r}
# 95 percentile of brain volume
qnorm(1-0.05, mean=mu, sd=sigma)
# Median
qnorm(0.5, mean=mu, sd=sigma)
# IQR
qnorm(0.75, mean=mu, sd=sigma) - qnorm(0.25, mean=mu, sd=sigma)
```
...
* Then 
\begin{align*}
\P(Y<z) = \P((X\sigma+\mu)<z) = \P(X< (z - \mu)/\sigma) \\
= \int_{-\infty}^{(z-\mu)/\sigma} \left(2\pi \right)^{-1/2} \exp\left\{-\frac{1}{2} x^2 \right\} dx\\
= \int_{-\infty}^{y} \left(2\pi \sigma^2\right)^{-1/2} \exp\left\{-\frac{1}{2\sigma^2} (w-\mu)^2 \right\} dw
\end{align*}
...
* The insurance dataset is in the `RESI` R package, which you can install with the following code
...
* Conditional probabilities are $\P(\text{N kids} | \text{Region})$.
...
Correlation is a normalized variance
\[
\begin{align*}
...
![correlation](figures/rice_correlation.png){#id .class width=100% height=100%}
...
\[
\begin{align*}
\hat\rho = \frac{\widehat{\text{Cov}}(X,Y)}{\sqrt{\widehat{\text{Var}}(X)\widehat{\text{Var}}(Y)}}
...
X = rnorm(100)
Y = X^2
plot(X, Y, main='Uncorrelated dependent no noise')
abline(lm(Y ~ X))
cor(X,Y)

X = rnorm(100)
Y = X^2 + rnorm(100)
plot(X, Y, main='Uncorrelated dependent with noise')
abline(lm(Y ~ X))
cor(X,Y)
```
...
![Startsleeping.org](figures/sleep-needs-infographic.png){#id .class width=30%}
...
```{r}
hcp = read.csv('../datasets/hcp/hcp.csv')
hist(hcp$PSQI_AmtSleep, main='Nightly sleep', xlab='Hours')
hcp$healthySleep = ifelse(hcp$PSQI_AmtSleep<=9 & hcp$PSQI_AmtSleep>=7, 1, 0)
```
...
2. What is my population of interest?
    ...
    + Quantitatively two possible things
      ...
      * Unknown statistical distribution that gives rise to how whether someone reports between  7-9 hours of sleep.
...
* The bias of the proportion estimator (used to estimate the proportion of people who get enough sleep) is:
\[
\E( \hat p - p) = 0
\]
...
#### HCP: example of bias, variance, and MSE
...
### Another example: multiplexed immunofluorescence (mIF) microscopy
